...
Created platform-app/hooks/use-portfolio-favorite.ts with the usePortfolioFavorite hook. It follows the exact same pattern as useVendorFavorite:
- **Check**: GET /favorites?targetId={portfolioId}&targetType=portfolio
- **Add**: POST /favorites with body { targetId, targetType: 'portfolio' }
- **Remove**: DELETE /favorites/{favoriteId}
- **Optimistic updates**: isFavorited and localCount update immediately, roll back on error
